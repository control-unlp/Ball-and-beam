...
const int trigPin = 6;    // Trig pin of the ultrasonic sensor (Yellow)
const int echoPin = 5;   // Echo pin of the ultrasonic sensor (Orange)
const int servoPin = 9;  // Servo Pin

float Kp = 0.5;                                                   //Initial Proportional Gain 2.05
...
void setup() {
  ...
  myPID.SetOutputLimits(-100,100);                             // Set output limits to match servo range
}
...
void loop(){
  ...
  ServoOutput = map(Output, -100, 100, 50, 170);
  ServoOutput = constrain(ServoOutput, 50, 170); 
